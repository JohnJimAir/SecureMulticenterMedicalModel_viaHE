...
func main() {
	// Reading data
	data_matrix_file_name := "../../data/3/breast-test_data_x_y_array1.txt"
	...
	for i := 1; i <= 3; i++ { // Layer i
		weight_matrix_file_name := "../../model/breast/Selu/W" + strconv.Itoa(i) + ".txt"
		bias_vector_file_name := "../../model/breast/Selu/b" + strconv.Itoa(i) +".txt"
	...
	if params, err = hefloat.NewParametersFromLiteral(
		hefloat.ParametersLiteral{
			LogN:            15,
			LogQ:            []int{51, 40, 40, 40, 40, 40, 40, 40, 40, 40, 40, 40, 40, 40},
			LogP:            []int{50, 50, 50},
			LogDefaultScale: 40,
		}); err != nil {
		panic(err)
	...
	for k := 0; k < len(X); k++ {
		...
		for i := 1; i <= 3; i++ { // The i-th Layer
			...
			// c_x := float64_to_complex128(x)

			// want := EvaluateLinearTransform(c_x, diagonals)
			// x = real_part_of_complex_array(want)
			...
			if i < 3 {
				...
				poly := hefloat.NewPolynomial(src.GetChebyshevPoly(K, 15, sigmoid))
	...
	fmt.Println("timing: ", time_elapsed)
	// fmt.Println("********************************************************************")
}