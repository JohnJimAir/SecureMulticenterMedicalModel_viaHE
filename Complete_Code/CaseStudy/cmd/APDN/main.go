...
func main() {
	...
	for i:=1;i<=3;i++{// Layer i
		for j:=1; j<=2; j++{// The j-th linear transformation
			weight_matrix_file_name := "model/W"+strconv.Itoa(i)+strconv.Itoa(j)+".txt"
			bias_vector_file_name := "model/b"+strconv.Itoa(i)+strconv.Itoa(j)+".txt"
	...
	for k := 0; k < len(X); k++{
		...
		for i:=1; i<=3; i++{// The i-th Layer
			...
			// fmt.Printf("vector x matrix %s", hefloat.GetPrecisionStats(params, ecd, dec, want_1, ct_z, 0, false).String())
