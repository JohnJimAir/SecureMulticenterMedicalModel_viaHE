...
func main() {
	// Reading data
	data_matrix_file_name := "data/data_x_y.txt"
	X, err1 := src.Read_matrix(data_matrix_file_name)
	if err1 != nil {
		fmt.Println(err1)
		return
	}
	...
	for i:=1;i<=3;i++{// Layer i
		for j:=1; j<=2; j++{// The j-th linear transformation
			weight_matrix_file_name := "model/EPDN/syn1/W"+strconv.Itoa(i)+strconv.Itoa(j)+".txt"
			bias_vector_file_name := "model/EPDN/syn1/b"+strconv.Itoa(i)+strconv.Itoa(j)+".txt"
			w, err1 := src.Read_matrix(weight_matrix_file_name)
			if err1 != nil {
				fmt.Println(err1)
				return
			}
			// fmt.Println("The number "+ strconv.Itoa(j)+ " weight matrix of layer " + strconv.Itoa(i) + ": ")
			// for _, row := range w {
			// 	fmt.Println(row)
			// }
			W = append(W, w);
	...
	for k := 0; k < len(X); k++{
		...
		//Encrypting
		ct_x_in, err := enc.EncryptNew(pt_x)
		if err != nil {
			panic(err)
		}
		...
		for i:=1; i<=3; i++{// The i-th Layer
			...
			fmt.Println("The first bias vector of layer " + strconv.Itoa(i) + ": ")
			fmt.Println(b)
			...
			if err := ltEval.Evaluate(ct_x_in, lt, ct_x); err != nil {
				panic(err)
			}
			...
			fmt.Println("\n Addition: \n Plaintext results after addition followed the first LT in layer " + strconv.Itoa(i) + ": ")
			for i := 0; i < n_rows && i < len(want); i++ {
				fmt.Print(want[i]+complex(b[i],0), ", ")
			}
			fmt.Println()

			fmt.Println("Encrypted results after addition followed the first LT in layer " + strconv.Itoa(i) + ": ")
			pt_res = dec.DecryptNew(ct_x)
			if err = ecd.Decode(pt_res, have); err != nil {
				panic(err)
			}
			...
			w_1 := W[(i-1)*2+1];
			b_1 := B[(i-1)*2+1];
			fmt.Println("\nThe second weight matrix of layer "+strconv.Itoa(i) +": ")
			for _, row := range w_1 {
				fmt.Println(row)
			}
			fmt.Println("The second bias vector of layer "+strconv.Itoa(i) +": ")
			fmt.Println(b_1)
			...
			if err := ltEval_1.Evaluate(ct_x_in, lt_1, ct_z); err != nil {
				panic(err)
			}
			...
			c_y := src.Float64_to_complex128(x);
			want_1 := src.EvaluateLinearTransform(c_y, diagonals_1)
			fmt.Println("Plaintext results after the second linear tranformation of layer " + strconv.Itoa(i) + ": ")
			for i := 0; i < n_rows_1 && i < len(want_1); i++ {
				fmt.Print(want_1[i], ", ")
			}
			fmt.Println()
			...
			fmt.Println("Encrypted results after the second linear tranformation of layer " + strconv.Itoa(i) + ": ")
			fmt.Println(have_1[:n_rows_1])
			...
			fmt.Println("\n Addition: \n Plaintext results after addition followed the second LT in layer " + strconv.Itoa(i) + ": ")
			for i := 0; i < n_rows && i < len(want); i++ {
				fmt.Print(want_1[i]+complex(b_1[i],0), ", ")
			}
			fmt.Println()

			fmt.Println("Encrypted results after addition followed the second LT in layer " + strconv.Itoa(i) + ": ")
			pt_res_1 = dec.DecryptNew(ct_z)
			if err = ecd.Decode(pt_res_1, have_1); err != nil {
				panic(err)
			}
			fmt.Println(have_1[:n_rows])


			// Multiplication
			fmt.Println("\n Multiplication: \n Plaintext results after multiplication in layer " + strconv.Itoa(i) + ": ")
			
			ct_x_in, err = eval.MulRelinNew(ct_x, ct_z)
			if err != nil {
				panic(err)
			}
			if err := eval.Rescale(ct_x_in, ct_x_in); err != nil {
				panic(err)
			}
			...
			// Decodes the plaintext
			pt_res_2 := dec.DecryptNew(ct_x_in)
			have_2 := make([]float64, ct_x_in.Slots())
